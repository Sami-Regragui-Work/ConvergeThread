                ...
                @forelse($duos as $duo)
                    <div class="flex items-center gap-4 px-5 py-4 hover:bg-white/5 transition group">
                        <a href="{{ route('messages.index', ['duo', $duo->id]) }}" class="flex items-center gap-4 flex-1 min-w-0">
                            <div class="flex -space-x-2 shrink-0">
                                @foreach([$duo->user1, $duo->user2] as $u)
                                    <div
                                        class="w-8 h-8 rounded-full border-2 border-surface-200 flex items-center justify-center text-xs font-semibold"
                                        :style="'background-color: ' + @js($u?->avatarColor() ?? '#64748b')">
                                        {{ $u?->avatarInitial() ?? '?' }}
                                    </div>
                                @endforeach
                            </div>
                        ...
                        @can('delete', [$group, $duo])
                            <form method="POST" action="{{ route('groups.duos.destroy', [$group, $duo]) }}" class="shrink-0">
                                @csrf @method('DELETE')
