@extends('layouts.app')
@section('title', 'Edit Role Override')

@section('content')
    <div class="max-w-lg mx-auto">
        <h1 class="text-xl font-bold text-white mb-6">Edit Role Override</h1>

        <div class="bg-surface-200 border border-white/5 rounded-2xl p-8 shadow-xl">
            <form method="POST" action="{{ url('/groups/' . $group->id . '/role-overrides/' . $override->id) }}"
                class="space-y-5">
                @csrf @method('PATCH')

                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-1.5">Base Tenant Role</label>
                    <select name="tenant_role_id" required
                        ...
                        @foreach($tenantRoles as $role)
                            <option value="{{ $role->id }}" {{ old('tenant_role_id', $override->tenant_role_id) == $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-1.5">Override Name</label>
                    <input type="text" name="name" value="{{ old('name', $override->name) }}" required
                ...
                <div class="flex gap-3 pt-2">
                    <a href="{{ url('/groups/' . $group->id . '/role-overrides') }}"
