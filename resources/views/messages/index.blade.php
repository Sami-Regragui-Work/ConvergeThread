...
@section('content')
    <div class="flex flex-1 min-h-0 flex-col px-4 py-3 sm:px-6"
        ...
        <div x-show="showCallModal" x-cloak class="fixed inset-0 z-200 flex items-center justify-center p-4">
            <div class="absolute inset-0 bg-black/60" @click="showCallModal = false"></div>
            <div class="relative bg-surface-200 border border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl text-center space-y-4">
                <div class="w-16 h-16 mx-auto rounded-full bg-brand-500/10 flex items-center justify-center">
                    <svg x-show="callType === 'voice'" class="w-8 h-8 text-brand-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                    </svg>
                    <svg x-show="callType === 'video'" class="w-8 h-8 text-brand-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/>
                    </svg>
                </div>
                <div>
                    <p class="text-white font-semibold" x-text="callType === 'video' ? 'Starting video call…' : 'Starting voice call…'"></p>
                    <p class="text-sm text-slate-500 mt-1">Connecting to {{ $chatable->name ?? 'chat' }}. WebRTC integration is in progress.</p>
                </div>
                <button type="button" @click="showCallModal = false"
                    class="w-full py-2.5 rounded-xl bg-red-500/20 text-red-400 hover:bg-red-500/30 text-sm font-semibold transition">End call</button>
            </div>
        </div>
    </div>
