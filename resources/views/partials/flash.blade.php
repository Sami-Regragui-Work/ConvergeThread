                ...
                <div @class([
                    ...
                    <div class="flex items-start gap-3">
                        <div class="min-w-0 flex-1 space-y-1">
                            @if ($flashMessage)
                                <p class="text-sm font-medium break-words">{{ $flashMessage }}</p>
                            @endif
        ...
        <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 5000)" x-show="show"
            ...
            @if (session('success'))
                <div
                    ...
                    <p class="text-sm min-w-0 flex-1 break-words">{{ session('success') }}</p>
                    <button @click="show = false" class="ml-auto shrink-0 text-emerald-400/60 hover:text-emerald-400 transition">
            ...
            @if (session('error'))
                <div
                    ...
                    <p class="text-sm min-w-0 flex-1 break-words">{{ session('error') }}</p>
                    <button @click="show = false" class="ml-auto shrink-0 text-red-400/60 hover:text-red-400 transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12" />
                        </svg>
            ...
            @if (session('info'))
                <div
                    ...
                    <p class="text-sm min-w-0 flex-1 break-words">{{ session('info') }}</p>
                    <button @click="show = false" class="ml-auto shrink-0 text-brand-400/60 hover:text-brand-400 transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12" />
                        </svg>
